	...
	const formatData = (resultDoc) => {
		if (resultDoc.result == 'not_published') {
			...
			return `Congratulations ${
				result.personalData.name
			}! You have been selected for the interview round for ${result.selectedDepartments.join(
				' and '
			)} department(s)`;
			...
			{loading ? (
				...
				<>
					...
					{result.result == 'selected' && (
						<>
							<h1 className={styles.selected}>
								...
								You have been selected for the interview round for the{' '}
								<span className={styles.span}>{dept}</span> department!
							</h1>
							<p className={styles.welcome}>
								Make sure to join the WhatsApp Group for Round 2{' '}
								<a
					...
					{result.result == 'rejected' && (
						<>
							<h1 className={styles.rejectionHeader}>
								We regret to inform you that you have not been selected this
								time. But don't lose hope, keep learning and improving your
								skills!
							</h1>
							<p className={styles.welcome}>
								We are thankful for the time you took to apply for CYSCOM and
								appear for the interview.
							</p>
							<p className={styles.welcome}>
								This is just the beginning, and you can most certainly apply
								again next year!
							</p>
							<p className={styles.welcome}>
